...
// Handle struct for I2Cx peripheral
typedef struct {
	I2C_RegDef_t *pI2Cx;
	I2C_Config_t I2C_Config;

	uint8_t *pTxBuffer;		// store tx buffer addr
	uint8_t *pRxBuffer;		// store the rx buffer addr
	uint8_t TxLen;			// tx length
	uint8_t RxLen;			// rx length
	uint8_t TxRxState;		// @PossibleTxRxStates
	uint8_t DevAddr;		//
	uint32_t RxSize;		//
	uint8_t Sr;				// repeated start
} I2C_Handle_t;

// @PossibleTxRxStates I2C App states
#define I2C_READY				0
